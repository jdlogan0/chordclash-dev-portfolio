...
Looking back, the reason the audio was so delayed was likely because I didn't prime it, which I later did for the all of the song audio in Blueprints for the sequence method. The audio could also be periodically stopped and played from the timer's current time at certain intervals to account for lag. I would still recommend a timer/datatable based system depending on how many game elements depend on the timing of the song. For example, if there are other visual elements that depend on the bpm of the song playing, it'd be good to have the timer call a function every beat rather than having to add a keyframe for every beat in every song sequence. This method also allows for more customization of gameplay, such as changing note speed, which is an important feature in particular for rhythm games that have a wide range of song difficulty.

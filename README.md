...
I first used [Reaper](https://www.reaper.fm/) to map out all of the times I wanted a note to play at a certain lane. These times were inputted into a Google Sheet and exported to a CSV.
<p align="center">
...
Looking back, the reason the audio was so delayed was likely because I didn't prime it, which I later did for the all of the song audio in Blueprints for the sequence method. The audio could also be periodically stopped and played from the timer's current time at certain intervals to account for lag. I would still recommend a timer/data table based system depending on how many game elements are reliant on the timing of the song. For example, if there are other visual elements that depend on the bpm of the song playing, it'd be good to have the timer call a function every beat rather than having to add a keyframe for every beat in every song sequence. This method also allows for more customization of gameplay, such as changing note speed, which is an important feature for rhythm games that have a wide range of song difficulty.

### Sequences

While I couldn't find many tutorials related to rhythm games in Unreal (particularly those not involving plugins), there were tutorials for timing events to music using sequences. I hadn't wanted to use this method at first mostly because I wanted to leave open the possibility of changing note speed (a feature that some rhythm games have so that notes don't get cramped in more difficult songs), which would be harder to implement in a sequence, where the tracks are relatively set in stone. I had also wanted to see if there was a method where the timing of notes didn't have to be directly edited in Unreal. As neither of these things were a huge priority, sequences were worth trying.

I still used Reaper to get the timing of notes correct, then created a sequence with an actor for spawning notes bound to it (this actor, NoteSpawner, handled most note-related functions). It had 4 trigger event tracks to easily keep track of which lanes a note was being fired from. All of the keys connected to an event in the NoteSpawner, with the lane as the only variable. When all of the keys were in the right spot, they were dragged back by two seconds so that they would hit the bar at the correct time after moving downwards. Besides being a very tedious task, I also had some trouble with getting the keys bound to the event. When I went into properties, the correct event didn't always show up, and I kept accidentally creating new events. Looking back (after doing the same process for multiple songs), there had likely been a key I was binding incorrectly and then copied, causing the issue to be present throughout the song.
...
There are 8 queues, with one per lane per player. When a note (an arrow object) is spawned it's added to the correct lane queue for both players. When the note is hit or moves past the area where it's considered a hit, it is removed from the queue. That way when a player uses a key to hit a note, the note is taken from the correct lane queue so it's position can be checked, and the accuracy assessment is handled accordingly.
...
The last method I tried with sequences was changing the bindings, which can be done at runtime. The idea was that the binding would change from a close actor to an actor far away and vice versa depending on health. There were a couple tutorials on this, so I had an idea of what needed to be done, but I wasn't able to successfully implement it. It's a method that I would like to go back to, as the main reason I moved on was for time's sake. 
...
1. **Search forums often** - If you're having an issue, chances are someone else has too. It's not a guarantee you'll find answers, but Unreal forums are often a good starting point.
...
6. **If you're working with timing events to audio, try different methods before settling on one!** There are a lot of ways you could go about doing this, and choosing the best method will likely depend on which parts of your game are tied to the music. If certain cues are tied to specific moments in a song (pyrotechnics at a particular line, point bonuses on the chorus), sequences are a good choice. If parts of the game are constantly updated by the music, like lights fluctuating to the beat of the song, timer intervals are probably a better option. And those are just the two main methods I tried; there are definitely more! Make sure to think ahead, since a method that initially seems like a good fit may not work for features you plan to implement later in the game. You don't want to be stuck in a position where you either have to scrap those features or scrap your whole system.
